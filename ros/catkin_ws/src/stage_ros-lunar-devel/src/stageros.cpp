...
//Server message
#include <stage_ros/resetToPose.h>

#include <std_srvs/Empty.h>

#include "tf/transform_broadcaster.h"
//For custome stall message
...
private:
    ...
    ros::ServiceServer reset_srv_, reset_w_pose_srv_;
...
public:
    ...
    bool cb_reset_to_pose_srv(stage_ros::resetToPose::Request& request, stage_ros::resetToPose::Response& response);
...
bool
StageNode::cb_reset_to_pose_srv(stage_ros::resetToPose::Request& request, stage_ros::resetToPose::Response& response)
{
  ROS_INFO("Resetting stage and moving robot!");
  Stg::Pose robot_pose;
  robot_pose.x = request.x;
  robot_pose.y = request.y;
  for (size_t r = 0; r < this->positionmodels.size(); r++) {
    this->positionmodels[r]->SetPose(robot_pose);
    this->positionmodels[r]->SetStall(false);
  }
...
StageNode::SubscribeModels()
{
	...
	// TODO - print the topic names nicely as well
        ...
        //new_robot->collision = n_.advertise<std_msgs::Bool>("collision", 10);
    ...
    reset_w_pose_srv_ = n_.advertiseService("reset_positions_with_pose", &StageNode::cb_reset_to_pose_srv, this);
...
StageNode::WorldCallback()
{
  if( ! ros::ok() ) {
    ...
    for (size_t r = 0; r < this->robotmodels_.size(); ++r)
    {
        ...
        //Publishes stall on a separate topic,  when one becomes available

        //stall_msg.data = this->positionmodels[r]->Stalled();
        //robotmodel->collision.publish(stall_msg);
        //
