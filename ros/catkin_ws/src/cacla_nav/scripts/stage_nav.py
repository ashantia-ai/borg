...
class CACLA_nav(object):
    def __init__(self, base_path, reference_frame,x_init,y_init, goal=(-3.53, -6.14, 0.0), ordered_list_index = [(0,0,0)], **kwargs):
        ...
        self.x_len = self.maxX + math.fabs(self.minX)
        self.y_len = self.maxY + math.fabs(self.minY)
        #self.metric_resolution = 0.5
        self.metric_resolution = 0.3
    ...
    def __launch_sim_and_world(self, ind):
        ...
        os.system('sh '+self.simul_path+'g_start.sh '+str(ind))
        time.sleep(2)
        print 'world created'
    ...
    def _convert_to_grid(self, x, y, theta):
        ...
        columns = int(math.ceil(self.x_len / self.metric_resolution))
        rows = int(math.ceil(self.y_len / self.metric_resolution))
