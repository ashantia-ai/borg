...
class CACLA_nav(object):
    ...
    def save_and_exit(self):
        ...
        try:
            fold = "{:.1f}".format(self.x_init) + '_' + "{:.1f}".format(self.y_init) + '/'
    ...
    def save_state(self, force = False):
        if (time.time() - self.save_timer > self.save_interval) or force:
            ...
            filepath = os.path.join(base_path, 'stage_single_goal_kitchen_state')
...
'''
This File is responsible to run the single goal 2D simulator for small Kitchen RL experiments.
...
if __name__ == '__main__':
    ...
    experiment._init_subscribers()
    experiment._init_ros_node()
