...
class CACLA_nav(object):
    ...
    def __init__(self, base_path, reference_frame, x_init, y_init, goal = (-3.53, -6.14, 0.0), ordered_list_index = [(0, 0, 0)], **kwargs):
        ...
        self.x_offset = 0.495  # Gazebo Offset X
        self.y_offset = -0.598  # Gazebo Offset Y  
    ...
    def _convert_from_grid(self, x, y):
        ...
        precise_y = (-x * step_size_x) - 0.2 - self.y_offset
        precise_x = (y * step_size_y) + 0.2 - self.x_offset
    ...
    def _perform_movebase_action_no_rot(self, action_num, state):
        ...
        # TODO: THIS IS NOT WORKING. IT IS PLANNING TO POSITIONS WHICH SHOULD NOT PLAN. DAFUQ
        # goal_pose, result = self._check_path(start_pose, x, y)
        '''   
        if not result:
        ...
        This if structure below is for the self._convert_from_grid(x, y) is called after it. Other wise x y locations should be reversed.
        if action_num == 0:
            x -= 1
        elif action_num == 1:
            y -= 1
    ...
    def _prepare_init_n_goal_position(self, goal = (-3.53, -6.14, 0.0),):
        ...
        self.goal_xy_threshold = 0.7  # Meters
