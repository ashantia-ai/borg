...
from g_stage_nav import CACLA_nav
...
class MultiGoalStage(CACLA_nav):
    ...
    def __init__(self, base_path, reference_frame, x_init, y_init, goals, ordered_list_index = [(0, 0, 0)], g_l_for_parent = [], full_info = [], **kwargs):
        ...
        self.nfq_args = {'epsilon':(0.1, 0.1) \
                ...
                , 'learning_rate':0.1, 'temperature':0.25, 'max_replay_size':50000}
        ...
        self.x_len = 11; self.y_len = 14  # FOR CAFFE 47 / 24
        ev_size = 154  # 1128
    ...
    def save_state(self, force = False):
        if (time.time() - self.save_timer > self.save_interval) or force:
            ...
            filepath = os.path.join(base_path, 'stage_multi_goal_kitchen_state')
...
'''
This File is responsible to run the multigo 2D simulator for small Kitchen RL experiments.
You HAVE to use a trained network otherwise this will TAKE FOR EVER.

Make sure that you used multi_goal.py for training. Afterwards, copy the goal_starts from correct folder 
