...
class MultiGoal(FAKE_test):
    ...
    def __init__(self, base_path, parent_conn, maze_path, maze_ind, results_path, **kwargs):
        ...
        self.avg_steps_number_difference = 0
        ...
        self.empty_vector = np.zeros((1, ev_size), dtype = theano.config.floatX) 
        self.empty_vector = np.reshape(self.empty_vector, -1)
        ...
        self.consecutive_success_steps = list(np.ones(15) * self.max_step_number)
        self.fig1_data = []       
        self.fig2_data = []
    ...
    def _easy_grid(self, x, y, goal_x, goal_y):
        ...
        pos_matrix = np.zeros((rows, columns), dtype = theano.config.floatX)
        pos_matrix[x, y] = 1.0
        pos_matrix = np.reshape(pos_matrix, -1)
        
        # If you want to have goal as input, uncomment here   
        '''
        goal_pos_matrix = np.zeros((rows, columns), dtype=theano.config.floatX)
        goal_pos_matrix[goal_x, goal_y] = 1.0
        goal_pos_matrix = np.reshape(goal_pos_matrix, -1) 
        #dot_product = np.dot(pos_matrix, goal_pos_matrix)
        
        h_stack = np.hstack((pos_matrix, goal_pos_matrix))
        '''
        return pos_matrix
    ...
    def select_active_goal(self):
        ...
        for i in range(len(self.goal_set)):
            ...
            goal_td = np.mean(self.RL.td_map[i][goal.x, goal.y, :])
            ...
            if self.running:
                ...
                if times_up or main_goal_reward >= self.angular_fix_reward:
                    ...
                    self.avg_steps_number_difference = math.fabs(self.last_avg_steps_number - 
                                                            np.asarray(self.consecutive_success_steps).min())
                    ...
                    print "min steps ", np.asarray(self.consecutive_success_steps).min()
                    print "avg steps mean ", np.asarray(self.avg_steps_lists).mean()
                    if self.avg_err > 0:
                        print "latest total avg XP replay err", self.avg_err
                    if (np.asarray(self.consecutive_succes).mean() >= 0.999  
                        # and np.asarray(self.consecutive_success_steps).mean() < self.average_step_number  
                        # and np.asarray(self.consecutive_success_steps).min() < self.max_step_number):
                        and np.asarray(self.avg_steps_lists).mean() <= 200
                        and self.avg_err < 0.00000001):
                    ...
                    self.last_avg_steps_number = np.asarray(self.consecutive_success_steps).mean()    
    ...
    def save_state(self, force = False):
        if (time.time() - self.save_timer > self.save_interval) or force:
            ...
            filepath = os.path.join(base_path, 'maze_multi_goal_kitchen_state')
    ...
    def _prepare_next_init(self):
        ...
        self.x_init, self.y_init = self.starts[self.init_index]
...
'''
Multi goal is supposed to solve multiple mazes using the MGNFQ approach. This file is for the small kitchen.
...
if __name__ == '__main__':
    ...
    base_path = os.path.join(os.environ['HOME'], "amir-nav-experiments/maze_files/small_kitchen") 
