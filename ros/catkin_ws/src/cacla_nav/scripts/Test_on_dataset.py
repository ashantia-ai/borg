...
class Test(object):
    ...
    def load_maze_and_goal(self, path, num, ind=0, ind2=0):

        self.structure = np.load(path+'maze_'+str(num)+'.npy')
        goal = np.load(path+'goal_'+str(num)+'.npy')
        starts = np.load(path+'starts_'+str(num)+'.npy')
        

        (x_goal, y_goal)=goal[ind]
        (x_start, y_start)=starts[ind2]
            ...
            if self.running:
                ...
                if times_up or reward >= self.angular_fix_reward:
                    ...
                    if np.asarray(consecutive_succes).mean() >= 0.999 and self.steps_number < 100:
                        ...
                        self.save_results(save_path, to_save, actions_numbers)
                        #time.sleep(1)
                        self.fig1_data=[]       
                        self.fig2_data=[]
                        consecutive_succes=list(np.zeros(50))
...
if __name__=='__main__':
    ...
    try:
        ...
        print 'saving path exist'
    
    dataset_num=1
    experiment = Test(base_path)
    
    for curr_data in xrange(dataset_num):
        for i in xrange(10):
            for j in xrange(10):
            
                experiment.load_maze_and_goal(dataset_path, curr_data, i, j)
                experiment.runner_batch(results_path)
                experiment.reset()
                print 'done starting point {} of goal: {}; of maze: {}'.format(curr_data, i ,j) 
