...
class Tester(object):
    ...
    def gmapping_image(self, input_image):
        ...
        labels_norm = 8.125
        # mean_x, mean_y
        labels_shift = [0.625, 8.125]
        #order: multiply by label_norm, then add shift
        '''
        old wrong way
        min_x = -0.25
        max_x = 4.5
        ...
        x_loc = (x_loc * max_abs)+mean_x
        y_loc = (y_loc * max_abs)+mean_y
    ...
    def update(self):
        if self.counter > 3:
         ...
         #   self.lock.release()
            ...
            self.location_array.header.frame_id
...
Model: Make sure your model to the path is correct. The best one we has is iteration703 from latest_models
REQUIRED CNN LINES: For each model you need the correct CNN layers in the main functions
IMAGE: Make sure you are correctly dividing the image. It is either image = image / 127.5 - 1 
...
#----------------------------------------------------------------------------------
    ...
    #model = '/home/ashantia/University/cnn/sim/regression/lr0.0001/0/model/iteration219/model.ckpt'
